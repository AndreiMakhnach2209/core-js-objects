...
function mergeObjects(objects) {
  return objects.reduce((acc, obj) => {
    ...
    Object.entries(clone).forEach((entr) => {
      if (acc[entr[0]]) clone[entr[0]] += acc[entr[0]];
    });
...
function sellTickets(queue) {
  let res = true;
  const cost = 25;
  const nominals = [25, 50, 100];
  const cash = nominals.reduce((acc, item) => {
    acc[item] = 0;
    return acc;
  }, {});
  queue.forEach((bill) => {
    ...
    Object.keys(cash)
      .sort((a, b) => b - a)
      .forEach((nom) => {
        const numberOfBill = Math.floor(change / nom);
        if (cash[nom] >= numberOfBill && change >= nom * numberOfBill) {
          cash[nom] -= numberOfBill;
          change -= nom * numberOfBill;
        }
      });
    if (change) res = false;
  });
...
class Builder {
  ...
  class(value) {
    if (this.hasAttr || this.hasPseudoElement || this.hasPseudoClass)
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    this.hasClass = true;
  ...
  attr(value) {
    if (this.hasPseudoElement || this.hasPseudoClass)
      throw new Error(
        'Selector parts should be arranged in the following order: element, id, class, attribute, pseudo-class, pseudo-element'
      );
    this.hasAttr = true;
